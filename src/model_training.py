        ...
        if self.model_type == 'logistic':
            ...
            try:
                ...
                self.model = XGBClassifier(
                    ...
                    learning_rate=0.1,
                    random_state=self.random_state,
                    scale_pos_weight=3,  # 正负样本比例，可后续根据实际调整
                    use_label_encoder=False,
                    eval_metric='auc',
                    n_jobs=-1
                )
            except ImportError:
                print("XGBoost 未安装，使用梯度提升替代")
    ...
    def train(self, X_train, y_train, use_smote=False):
        ...
        if use_smote:
            try:
                ...
                print(f"SMOTE 过采样后: {len(y_train)} 样本")
                print(f"正样本比例: {y_train.mean():.2%}")
            except ImportError:
                print("imbalanced-learn 未安装，跳过 SMOTE")
...
if __name__ == '__main__':
    # 简单测试：使用 Home Credit 真实数据的前 n 行做快速验证
    import os
    import sys
    ...
    if not os.path.exists(DATA_CONFIG['train_file']):
        ...
        # 只读前 5000 行做测试
        train_df = pd.read_csv(DATA_CONFIG['train_file'], nrows=5000)
